...
ROLES_PAGO_COLUMNS = [
    ...
    ('bonos', 'BONOS'),
    ('toting', 'TOTAL INGRESO'),
    ('ingconaporte', 'INGRESO CON APORTE AL IESS'),
    ('aporte', 'APORTE IESS'),
    ('dnlprestamos', 'D.N.L. O PRÉSTAMOS'),
    ('fracu', 'ACUMULACIÓN F.R.'),
    ('totregr', 'TOTAL EGRESOS'),
    ('liquido', 'LÍQUIDO A RECIBIR'),
]


def _roles_pago_datos(request):
    cliente = _cliente_portal(request)
    if cliente is None:
        return None, [], [], []
    ...
    mes_param = request.GET.get('mes', '').strip().upper()
    ...
    with db.cursor() as cursor:
        cursor.execute("""
            ...
            ORDER BY TRIM("año"::text) DESC
        """)
        anios = [row[0] for row in cursor.fetchall()]

    anio = anio_param if anio_param in anios else (
        anio_actual if anio_actual in anios else (anios[0] if anios else anio_actual)
    )

    meses_validos = {value for value, _ in meses if value}
    if mes_param not in meses_validos:
        mes_param = ''
    ...
    return {
        ...
        'anio': anio,
        'mes': mes_param,
        'mes_nombre': next((label for value, label in meses if value == mes_param), 'Todos los meses'),
    }, filas, anios, meses
...
def sri_anexos(request):
    ...
    raiz_cliente = Path(settings.TOTALCOUNTS_DATA_ROOT) / str(cliente.ruccedcli) / 'anexos'
    tipos = [
        {'nombre': 'Anexo Transaccional Simplificado', 'carpeta': 'ats', 'icono': 'fi-rr-document'},
        {'nombre': 'Anexo Relación Dependencia', 'carpeta': 'relacion_dependencia', 'icono': 'fi-rr-users'},
        {'nombre': 'Anexo Patrimonial', 'carpeta': 'patrimonial', 'icono': 'fi-rr-building'},
        {'nombre': 'Anexo ICE', 'carpeta': 'ice', 'icono': 'fi-rr-box'},
        {'nombre': 'Anexo ADI', 'carpeta': 'adi', 'icono': 'fi-rr-folder'},
        {'nombre': 'Anexo REBEFICS', 'carpeta': 'rebefics', 'icono': 'fi-rr-file'},
    ]
    ...
    for tipo in tipos:
        ...
        if carpeta.exists() and carpeta.is_dir():
            for anio_dir in carpeta.iterdir():
                if not anio_dir.is_dir() or not anio_dir.name.isdigit():
                    continue
                anio = anio_dir.name
                anios.add(anio)
                meses = []
                for mes_dir in sorted(anio_dir.iterdir(), key=lambda p: p.name):
                    if not mes_dir.is_dir() or not mes_dir.name.isdigit():
                        continue
                    archivos = []
                    for archivo in sorted(mes_dir.iterdir(), key=lambda p: p.name.lower()):
                        if archivo.is_file():
                            stat = archivo.stat()
                            archivos.append({
                                'nombre': archivo.name,
                    ...
                    if archivos:
                        meses.append({
                            'numero': mes_dir.name.zfill(2),
                            'nombre': dict([
                                ('01','Enero'),('02','Febrero'),('03','Marzo'),('04','Abril'),
                                ('05','Mayo'),('06','Junio'),('07','Julio'),('08','Agosto'),
                                ('09','Septiembre'),('10','Octubre'),('11','Noviembre'),('12','Diciembre')
                            ]).get(mes_dir.name.zfill(2), mes_dir.name),
                            'archivos': archivos,
                            'total': len(archivos),
                        })
                if meses:
                    meses.sort(key=lambda item: item['numero'], reverse=True)
                    grupos.append({'anio': anio, 'meses': meses})
        grupos.sort(key=lambda item: item['anio'], reverse=True)
        tipo['anios'] = grupos
        tipo['total'] = sum(m['total'] for a in grupos for m in a['meses'])

    return render(request, 'sri-anexos.html', {
        'cliente': cliente,
        'tipos_anexos': tipos,
        'anios_anexos': sorted(anios, reverse=True),
    })
